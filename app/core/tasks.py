...
DJANGO_CELERY_NET_MONITOR_INFO = "dblog-net-monitor-info-{network}-{time}"
DJANGO_CELERY_NET_MONITOR_INFO_SHOW = "dblog-net-monitor-info-show-{network}"
...
def network_monitor_info():
    ...
    for k in key_info:
        ...
        res = (redis.pipeline()
               ...
               .execute())
        stat_net = res[-1]
        set_network_monitor_info(redis, reversed(stat_net), k, bnow_fmt, bbnow_fmt)

def set_network_monitor_info(redis, nstat_net, net, bnow_fmt, bbnow_fmt):
...
def get_network_monitor_info():
    ...
    for k in key_info:
        d = _get_network_monitor_info(redis, k)
        if d:
            keys.append(k)
            j.append( (k, d) )
    return keys, j

def _get_network_monitor_info(redis, net):
    key = DJANGO_CELERY_NET_MONITOR_INFO_SHOW.format(network=net)
    j = redis.get(key)
    if not j: return None
    d = json.loads(j)
    ...
    caption = _(u'%(name)s网卡流量监控') % {'name': net}
    js = {
        'type': 'zoomline',
        'renderAt': 'tcp-info-container-{}'.format(net),
        'width': '100%',
        'height': '500',
        'dataFormat': 'json',
        'dataSource': {
